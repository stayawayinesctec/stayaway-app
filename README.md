...
# Introduction
[STAYAWAY COVID](https://stayaway.inesctec.pt/en/) is a voluntary system aiming at supporting the country in the screening of COVID-19. This repository contains the STAYAWAY COVID source code for both Android and iOS. Due to regulatory restrictions, not everyone can fully run the app, although we provide an UI only mode as detailed in the [building section](#building). So, if not everyone can run it, why are we open-sourcing this? It's pretty simple actually, we feel that such an important application should be available to the community. We welcome any bug reports, security or privacy concerns that you might have with the app and will do our best to address them.
